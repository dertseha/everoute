...
type PathContest struct {
	rule                  TravelRule
	pathsByDestinationKey map[string]*Path
}

func NewPathContest(rule TravelRule) *PathContest {
	contest := &PathContest{
		rule: rule,
...
func (contest *PathContest) Enter(path *Path) bool {
	var result = true
...
func (contest *PathContest) isPathStillCurrent(path *Path) bool {
	var entry = path
	var result = false
